  ...
  return {
    ...
    distribution_center: {
      ...
      funnel_tracking: [
        ...
        { stage: 'Started', value: 0 },
        { stage: 'Completed', value: 0 },
        { stage: 'AI processed', value: 0 }
      ]
    },
    ai_distribution_engine: {
      active_mode: 'hybrid',
      active_policy: 'balanced',
      channels: [
        { label: 'Phone Call', provider: 'Twilio Voice', autonomous: true },
        { label: 'WhatsApp Voice Notes', provider: 'WhatsApp Business / Twilio', autonomous: true },
        { label: 'WhatsApp Chat', provider: 'WhatsApp Business / Twilio', autonomous: true },
        { label: 'SMS / Feature-Phone Fallback', provider: 'Twilio SMS', autonomous: true },
        { label: 'Web Platform Link', provider: 'VoiceInsights Web', autonomous: true },
        { label: 'Offline Mobile Application', provider: 'VoiceInsights Offline App', autonomous: false }
      ],
      collection_modes: [
        { label: 'Autonomous AI Mode', description: 'AI chooses channels automatically.' },
        { label: 'Manual Distribution', description: 'Operator chooses exact channels.' },
        { label: 'Enumerator-led Collection', description: 'Offline Mobile App first.' },
        { label: 'Hybrid Mode', description: 'AI channels first, Enumerator fallback.' }
      ],
      campaign_policies: [
        { label: 'Balanced', optimizes: ['cost', 'speed', 'quality', 'response rate'] },
        { label: 'Voice First', optimizes: ['phone call', 'WhatsApp voice'] },
        { label: 'Enumerator First', optimizes: ['offline mobile app'] }
      ],
      automatic_selection: {
        route_logic: [
          'Start with preferred respondent channel.',
          'If phone is unanswered, retry.',
          'Switch to WhatsApp or SMS.',
          'Assign Enumerator through Offline App when digital channels fail.'
        ]
      }
    },
    me_workspace: {
...
function renderKpis(items = []) {
  return `<div class="v207c-kpis">${items.map(i => `<div class="v207c-kpi"><strong>${i.value ?? '—'}</strong><span>${i.label}</span></div>`).join('')}</div>`;
}

function renderChannels(channels = []) {
  return channels.map(c => `<div class="v207c-channel"><span class="v207c-dot"></span><div><strong>${c.label}</strong><br><small>${c.provider || ''} • ${c.autonomous ? 'Autonomous' : 'Enumerator / offline'}</small></div><span class="v207c-pill">Operational</span></div>`).join('');
}

function renderModes(modes = []) {
  return modes.map(m => `<div class="v207c-mode-card ${/Hybrid|Autonomous/i.test(m.label) ? 'recommended' : ''}"><strong>${m.label}</strong><p>${m.description || ''}</p></div>`).join('');
}
...
async function initV207CFieldIntelligence() {
  const root = document.getElementById('v207c-root');
  if (!root) return;

  const data = await loadV207CWorkspace();
  const d = data.distribution_center || {};
  const ai = data.ai_distribution_engine || {};
  const me = data.me_workspace || {};
  const en = data.enumerator_workspace || {};
  const safePublicLink = productionSurveyLink(d.public_link);

  root.innerHTML = `
    <div class="v207c-shell">
      <section class="v207c-hero">
        <p style="margin:0 0 8px;font-weight:900;color:#fef3c7">${data.official_positioning || 'VoiceInsights Africa'}</p>
        <h1>Autonomous Omni-Channel Intelligence Collection Engine</h1>
        <p>${data.business_promise || ''}</p>
        <div class="v207c-actions">
          <a class="v207c-btn primary" href="#distribution-center">Share Survey</a>
          <a class="v207c-btn" href="#campaign-builder">Launch Campaign</a>
          <a class="v207c-btn" href="#enumerator">Enumerator Workspace</a>
          <a class="v207c-btn dark" href="/app/report-library.html">Generate Reports</a>
        </div>
      </section>

      <nav class="v207c-tabs">
        <button class="v207c-tab active" data-v207c-tab="overview">Overview</button>
        <button class="v207c-tab" data-v207c-tab="channels">Channels</button>
        <button class="v207c-tab" data-v207c-tab="distribution">Distribution</button>
        <button class="v207c-tab" data-v207c-tab="enumerator">Enumerator</button>
        <button class="v207c-tab" data-v207c-tab="voice">Voice AI</button>
      </nav>
      ...
      <section data-v207c-panel="distribution" style="display:none">
        <div class="v207c-grid">
          <div class="v207c-card full" id="distribution-center">
            <h2>Survey Distribution Center</h2>
      ...
      <section data-v207c-panel="enumerator" style="display:none">
        <div class="v207c-grid">
    ...
    <div class="v207c-mobile-bar">
      <a href="#distribution-center">Share</a>
      <a href="#campaign-builder">Launch</a>
      <a href="#enumerator">Offline</a>
    </div>
  `;
...
window.initV207CFieldIntelligence = initV207CFieldIntelligence;
document.addEventListener('DOMContentLoaded', initV207CFieldIntelligence);
