...
async function renderFullFlagshipPdf(composition={},options={}) {
  ...
  addPage([`${pdfColor(cover.primary)} rg`,`0 0 ${W} ${H} re f`,`${pdfColor(cover.accent)} rg`,`390 0 205 ${H} re f`,`${pdfColor(cover.highlight||'#D4AF37')} rg`,`360 0 18 ${H} re f`,'0.110 0.620 0.330 rg','52 782 5 20 re f','61 774 5 36 re f','70 766 5 52 re f','79 774 5 36 re f','88 782 5 20 re f','BT','/F2 11 Tf','52 755 Td','(VOICEINSIGHTS AFRICA) Tj','/F1 10 Tf','0 -28 Td','(FULL SYNTHETIC DEMONSTRATION PUBLICATION) Tj','/F2 31 Tf','0 -70 Td',...wrapPdf(title,31).slice(0,5).flatMap((l,i)=>[i?'0 -38 Td':'',`(${escapePdfText(l)}) Tj`]).filter(Boolean),'/F1 14 Tf','0 -45 Td',`(${escapePdfText(`${f.country||''} • ${f.sector||''} • ${Number(f.sample_size||0).toLocaleString()} synthetic responses`)}) Tj`,'/F1 9 Tf','0 -55 Td','(Synthetic demonstration data. Not official statistics or real population findings.) Tj','ET']);
  ...
  for(const stream of pageStreams){const contentId=addObj(`<< /Length ${bytes(stream).length} >>\nstream\n${stream}\nendstream`);pageIds.push(addObj(`<< /Type /Page /Parent ${pagesId} 0 R /MediaBox [0 0 ${W} ${H}] /Resources << /Font << /F1 ${fontId} 0 R /F2 ${boldId} 0 R >> >> /Contents ${contentId} 0 R >>`))}
  ...
  const header='%PDF-1.7\n%\xE2\xE3\xCF\xD3\n';let body=header;const offsets=[0];objects.forEach((o,i)=>{offsets.push(bytes(body).length);body+=`${i+1} 0 obj\n${o}\nendobj\n`});const xo=bytes(body).length;body+=`xref\n0 ${objects.length+1}\n0000000000 65535 f \n`;for(let i=1;i<=objects.length;i++)body+=`${String(offsets[i]).padStart(10,'0')} 00000 n \n`;body+=`trailer\n<< /Size ${objects.length+1} /Root ${catalogId} 0 R /Info << /Title (${escapePdfText(title)}) /Producer (VoiceInsights Full Flagship Renderer v3) >> >>\nstartxref\n${xo}\n%%EOF`;const data=bytes(body);return{renderer_version:'flagship',format:'pdf',binary_generated:true,bytes:data,byte_length:data.length,checksum:await sha256Hex(data),content_type:'application/pdf',file_extension:'pdf',filename:`${sanitizePath(report.id||'voiceinsights-full-flagship')}.pdf`,quality:{selectable_text:true,page_count:pageStreams.length,cover_page:true,vector_infographics:true,full_publication:true}};
}
