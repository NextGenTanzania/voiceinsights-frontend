...
async function loadApiWorkspace(){
  ...
  try{
    ...
    target.innerHTML=`<div class="api-grid">${w.modules.map(m=>`<div class="api-card"><span class="api-pill">Configured</span><h3>${escapeHtml(m)}</h3><p>Integrated into the VoiceInsights developer platform.</p></div>`).join('')}</div>`;
