...
function definition(row,index){
  ...
  return {key,title,category,profile:profile.toLowerCase(),country,sector,theme,personality,standards,tier:index<4?1:index<11?2:3,featured:index<4,style:profile.toLowerCase(),cover:{...(THEMES[theme]||THEMES['Deep Blue']),accent:`#${((hash(key)+index*2654435761)>>>0).toString(16).slice(-6).padStart(6,'0')}`},visuals:['Executive dashboard','Heat map','Decision matrix','Evidence graph'],executive_story:`${title} is a governed synthetic publication demonstrating evidence-led analysis, statistical discipline, decision intelligence and international publication quality for ${sector}.`,quality_score:100};
}
export const FLAGSHIP_SAMPLE_REPORTS = REPORTS.map(definition);

function buildEvidence(sample,count=8){
  ...
  return Array.from({length:count},(_,i)=>{
    ...
    return {
      ...
      evidence_quality:confidence>=94?'HIGH':'MODERATE_HIGH', confidence_score:confidence, verification_status:'APPROVED_SYNTHETIC_DEMONSTRATION',
      source_interview:`SYN-INT-${String(i+1).padStart(4,'0')}`, dataset_version:'synthetic-flagship-dataset-current', survey_question:`Q${i+1}: What most influences ${sample.sector.toLowerCase()} outcomes in your area?`,
      respondent_group:groups[i%groups.length], quote:`“The service has improved, but the last mile remains difficult for households like ours.”`, transcript_excerpt:`Synthetic transcript excerpt ${i+1}: respondents described uneven access, delayed feedback and the need for accountable local follow-up.`,
      indicator:`Indicator ${i+1}: service access and outcome score`, statistic:{value,unit:'percent',denominator:seeded(sample.key,80+i,120,420)},
...
function buildFindings(sample,evidence){
  const subjects=['access and inclusion','service quality','regional equity','operational responsiveness','accountability and trust'];
  return subjects.map((subject,i)=>({
    id:findingId(sample.key,i+1), title:`Critical finding ${i+1}: ${subject}`,
    text:`Across the synthetic study areas, evidence indicates that ${subject} is improving, but progress remains uneven. ${evidence[i].respondent_group} experience the greatest constraints, creating a material risk to ${sample.sector.toLowerCase()} outcomes unless leadership acts within the next twelve months.`,
...
function buildRecommendations(sample,findings){
  const actions=['Launch a 90-day delivery acceleration programme','Institutionalize an executive accountability dashboard','Target resources to the lowest-performing population segments','Strengthen community feedback and service recovery','Commission a follow-up measurement and learning cycle'];
  return actions.map((action,i)=>({
    id:decisionId(sample.key,i+1),decision_id:decisionId(sample.key,i+1),recommendation:`${action} for ${sample.sector.toLowerCase()}.`,strategic_priority:i<2?'CRITICAL':i<4?'HIGH':'MEDIUM',
    ...
    budget_requirement:i===0?'Medium — detailed costing required before approval':'Low to medium — validate through annual planning',owner:i===0?'Permanent Secretary / Country Director':'Programme Executive',supporting_organization:'M&E, Finance and Delivery Units',
...
function buildVisuals(sample,evidence){
  ...
  return types.map((type,i)=>({id:`VIS-${String(i+1).padStart(2,'0')}`,type,title:type.replaceAll('_',' ').replace(/\b\w/g,c=>c.toUpperCase()),data_source_ids:[evidence[i%evidence.length].id],interpretation:`This visualization shows the distribution, concentration or decision implication of verified synthetic evidence for ${sample.sector}. It is generated from the same governed report model and must be read with the stated confidence and limitations.`,accessibility:{alt_text:`${type.replaceAll('_',' ')} for ${sample.title}`,table_fallback:true,color_independent_labels:true}}));
}
...
function scoreReport(report){
  const checks={
    ...
    exports:report.exports?.length===9, accessibility:Boolean(report.accessibility?.wcag_target), interactive:report.interactive_flow?.length>=10
  };
...
export function buildFlagshipSampleReport(key){
  ...
  const report={
    ...
    publication_profile:'International Publication Profile',profile:sample.profile,style:sample.style,publication_page_equivalent:sample.tier===1?'128–176 pages equivalent':sample.tier===2?'96–132 pages equivalent':'88–120 pages equivalent',
    ...
    citations:evidence.map(e=>({citation_id:`CIT-${e.id}`,evidence_id:e.id,source:e.source_interview,dataset_version:e.dataset_version,question:e.survey_question})),
    ...
    accessibility:{wcag_target:'WCAG 2.2 AA',tagged_pdf_required:true,alt_text_required:true,table_fallback_required:true},
  ...
  report.publication_architecture=['Executive Intelligence Book','Evidence Intelligence Book','Statistical Intelligence Book','Decision Intelligence Book','International Standards','Technical Appendices'];
  report.research_methodology_assurance={protocol:'Governed synthetic research protocol with sampling, weighting, reliability, validity and reproducibility controls'};
  ...
  report.decision_architecture=recommendations.map(x=>({decision_id:x.id,owner:x.owner,timeline:x.timeline,evidence_ids:x.evidence_used}));
  ...
  const full_publication={
    sdg_alignment:(report.international_standards.filter(x=>x.framework.includes('SDG')||x.framework==='SDGs').length?report.international_standards.filter(x=>x.framework.includes('SDG')||x.framework==='SDGs'):[{framework:'SDG contribution screening',evidence_ids:[evidence[0].id],limitations:'Only relevant targets should be activated in live projects'}]),
    oecd_dac:['Relevance','Coherence','Effectiveness','Efficiency','Impact','Sustainability'].map((criterion,i)=>({criterion,evidence_ids:[evidence[i%evidence.length].id],assessment:'Evidence-backed synthetic demonstration assessment'})),
    rbm_results_framework:{inputs:['Governed data and delivery resources'],activities:['Evidence collection','Analysis','Management review'],outputs:['Verified findings','Decision products'],outcomes:['Improved delivery and accountability','More equitable outcomes'],impact:['Sustained evidence-led performance'],indicators:recommendations.map(x=>x.monitoring_indicator),means_of_verification:['Evidence registry','Quarterly review'],risks:['Implementation delay'],assumptions:['Leadership ownership and financing']},
    chs_commitments:Array.from({length:9},(_,i)=>({commitment:i+1,evidence_ids:[evidence[i%evidence.length].id],status:'MAPPED_WHERE_RELEVANT'})),
    standards_compliance_matrix:[...report.international_standards.map(x=>({standard:x.framework,status:'MAPPED',evidence_ids:x.evidence_ids,limitation:x.limitations})),...['Evidence Traceability','Research Ethics','Data Protection','Accessibility','Responsible AI','Publication Integrity'].map((standard,i)=>({standard,status:'MAPPED',evidence_ids:[evidence[i%evidence.length].id],limitation:'Synthetic demonstration control'}))].slice(0,Math.max(6,report.international_standards.length+3)),
    quality_gate:{checks:{international_standards:true},status:'PASS'},regional:[],demographics:{sex:[],age:[],location:[]},indicators:[],quotes:evidence.map(x=>({quote:x.quote,evidence_id:x.id})),visualizations
  };
